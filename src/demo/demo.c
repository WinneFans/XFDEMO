...
int main(int argc, char* argv[])
{
	...
	const char* login_params = "appid = 57870361, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	/*
	* sub:			请求业务类型
	* domain:		领域
	* language:		语言
	...
	*/
	...
	while(1)
	{
		printf("是否开始录音?1开始,0退出\n");
		int recres = 0;
		scanf("%d", &recres);
		if(recres == 1)
		{
			printf("录音过程将持续10秒，现在开始...\n");
			system("rm ./wav/iflytek02.wav");
			system("arecord -d 10 -t wav -c 1 -r 16000 -f S16_LE ./wav/iflytek02.wav");
			printf("录音结束...\n");
